...
public class DataProvider implements CommandLineRunner {
    ...
    private void generateUser() {
        ...
        user1.setEmail("alikatkar@gmail.com");
        User user2 = new User("veli", passwordEncoder.encode("123"), Role.ROLE_ADMIN);
        user2.setEmail("alikatkar@gmail.com");
        
        userRepository.save(user1);
        userRepository.save(user2);
    }

    private void generateHotelBookings() {
        List<User> users = userRepository.findAll();
        List<HotelBooking> list = new ArrayList<>();

        for (int i = 0; i < hotels.length; i++) {
            list.add(new HotelBooking(users.get(i % users.size()), hotels[i], prices[i], 1));
        }

        bookingRepository.save(list);
    }

    @Override
    public void run(String... strings) throws Exception {
        generateUser();
        generateHotelBookings();
    }
}
